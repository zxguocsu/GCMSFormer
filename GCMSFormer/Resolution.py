...
def Resolution(path, filename, model, tgt_vacob, device):
    ...
    for i in range(num):
        ...
        ind_0 = np.argwhere(np.sum(xX, 1) >= 3.0 * noise)[:, 0]
    ...
    for j in tqdm(range(len(ls)), desc=filename):
        ...
        if len(pred_tgt_ind) > len(new_num):
            m0 = np.array(ncr.mat(ls[j][0] + 4, ls[j][-1]).T, dtype='float32')
        ...
        elif len(pred_tgt_ind) < len(new_num) and new_num[-1] > 0.2:
            m1 = m0[2:-2]
            t1 = t0[2:-2]
            X1, xX1 = converts(m1)
            new_num1 = []
