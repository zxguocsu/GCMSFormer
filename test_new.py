...
if __name__ == "__main__":
    ...
    loss_name = 'loss'
    mz_range = (1, 1000)
    if check_model(model_path + '/' + model_name, model_path + '/' + loss_name):  # 判断  '../model/GCMSFORMER' 和'../model/loss'是否存在
        # load model
        m_state_dict = torch.load(model_path + '/' + model_name + '.pt')  # '../model/GCMSFORMER.pt'
        model = make_model(len(tgt_vacob), N=6, d_model=1000, d_ff=1024, h=5, dropout=0.1)
        model.load_state_dict(m_state_dict)
        # load loss
        with open(model_path + '/' + loss_name + '.pk', 'rb') as file_1:  # ‘rb’：表示以二进制方式读取文件。该文件必须已存在。
